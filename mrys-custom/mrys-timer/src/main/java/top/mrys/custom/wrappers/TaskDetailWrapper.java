 ...
 * @param <P> 任务详情参数的类型。
 */
public abstract class TaskDetailWrapper<P> implements TaskDetail {
  // 任务基础详情对象，封装了任务的详细信息和行为。
  private final BaseTaskDetail<P> taskDetail;

  /**
   * 构造函数，用于初始化任务详情包装器。
   * @param taskDetail 任务的基础详情对象，不可为null。
   */
  public TaskDetailWrapper(BaseTaskDetail<P> taskDetail) {
   ...
   */
  public BaseTaskDetail<P> getTaskDetail() {
    return taskDetail;
  }
