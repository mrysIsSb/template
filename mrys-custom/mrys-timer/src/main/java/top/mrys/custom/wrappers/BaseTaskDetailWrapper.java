 ...
 * @param <P> 任务详情的泛型参数类型
 */
public class BaseTaskDetailWrapper<P> extends TaskDetailWrapper<P> {

  /**
   * 构造函数，初始化BaseTaskDetailWrapper。
   * @param taskDetail 任务详情对象，不允许为null。
   */
  public BaseTaskDetailWrapper(BaseTaskDetail<P> taskDetail) {
    super(taskDetail);
  }
  ...
  public Optional<TaskDetail> call() throws Exception {
    ...
    // 增加任务执行次数
    taskDetail.setExecTimes(taskDetail.getExecTimes() + 1);
    // 执行任务并获取结果
