...
public class ApiDocAutoConfiguration {
  ...
  public OpenApiProvider openApiProvider(Optional<List<ApiInfoCustomizer>> apiInfoCustomizers,
    ...
    return () -> {
      ...
      apiInfoCustomizers.ifPresent(customizers -> customizers.forEach(customizer -> customizer.customize(info)));
      api.info(info);
      //server
      List<Server> servers = Collections.emptyList();
      apiServerCustomizers.ifPresent(customizers -> customizers.forEach(customizer -> customizer.customize(servers)));
      api.servers(servers);
      //paths
      Paths paths = new Paths();
      apiPathsCustomizers.ifPresent(customizers -> customizers.forEach(customizer -> customizer.customize(paths)));
      api.paths(paths);
      //tags
      List<Tag> tags = Collections.emptyList();
      apiTagsCustomizers.ifPresent(customizers -> customizers.forEach(customizer -> customizer.customize(tags)));
      api.tags(tags);
